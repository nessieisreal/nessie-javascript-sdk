...
require(['customer'], function (customer) {
	...
	test('tests', function() {
		...
		ok(testCreateCustomer(customers, newCustDetails), 'Test Create a customer'); // Endpoint: 404 Error
...
function testGetCustomers(customers) {
	return customers.getCustomers().length > 0 && equals(customers.getCustomers()[0].first_name, 'John') || 
		   equals(customers.getCustomers().length, 0);
}
