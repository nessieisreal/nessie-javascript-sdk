...
require(['transfer', 'account'], function (transfer, account) {
	...
	test('tests', function() {
		...
		var paymentType = ["payer", "payee", "p2p"];

		var accountID = getFirstAccountId(accounts);
		ok(testGetAllTransfers(transfers, accountID, paymentType[0]), 'Test Get all transfers');
		...
		ok(testTransferById(transfers, transferID, paymentType[0]), 'Test Get transfer by id');
